...
separate explanation technique (SHAP, LIME) required. Trees also handle
both numerical and categorical features natively and require no feature
scaling -- both properties are exploited directly in Section~\ref{sec:c}
below.
...
\citep{mitchell1997machine, breiman1984classification}. This is exactly
what is observed for the baseline model in Section~\ref{sec:d}, and is the
motivation for the pruning improvement in Section~\ref{sec:f}.
...
\begin{enumerate}
  ...
  \begin{itemize}
    \item \textbf{Pruning} (cost-complexity pruning) -- addresses
      overfitting, trading tree complexity for generalization.
    \item \textbf{Class-imbalance handling}
      (\texttt{class\_weight='balanced'}, SMOTE) -- addresses the model's
      default bias toward the majority class (Graduate), which causes it
      to miss the minority classes that matter most for the application
      (Enrolled, Dropout).
    \item \textbf{Feature selection for early warning} -- addresses a
      ...
      known \emph{after} a semester ends, so the most accurate model is
      not necessarily the model that is actually deployable for early
      intervention.
  \end{itemize}
  \item Quantitatively compares all four models and draws conclusions
    not only about the numbers, but about \textbf{which model suits
    which application goal} -- in the spirit the assignment brief
    emphasizes: not just running a model, but explaining how the tree is
    built, how it behaves, and why each improvement helps (or does not).
\end{enumerate}
