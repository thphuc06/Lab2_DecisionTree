% Owner: Role D.
%
% Source material (fact-checked against notebooks/04_improve_imbalance.ipynb
% cell outputs, outputs/results.csv, outputs/classification_report_M2a.txt,
% outputs/classification_report_M2b.txt): docs/report_draft_f2_imbalance.md.
% All figures below are already committed under figures/ (filenames unchanged).
%
% Required bibliography keys used in this section (add to the shared .bib
% file if not already present -- suggested entries at the end of this file
% as a comment block for the integration owner):
%   chawla2002smote, imblearn_smote_api, imblearn_over_sampling_guide,
%   imblearn_arrays_transformer
...
maximising the number of correct predictions on the majority class.
...
(\textbf{M2b}). Both configurations reuse the single shared
train/test split produced by \texttt{src.data.get\_train\_test()}
...
\texttt{DecisionTreeClassifier} with \texttt{class\_weight=`balanced'} and
...
\begin{quote}
\texttt{X\_train\_smote, y\_train\_smote = SMOTE(sampling\_strategy="auto",
random\_state=42, k\_neighbors=5).fit\_resample(X\_train, y\_train)} \\
\texttt{DecisionTreeClassifier(random\_state=42).fit(X\_train\_smote, y\_train\_smote)}
\end{quote}
All three SMOTE hyperparameters are declared explicitly rather than left to their
library defaults, so that the resulting artefact does not silently change if a
future \texttt{imbalanced-learn} release alters its default behaviour
...
split and is fit exclusively on \texttt{X\_train}/\texttt{y\_train}. Because
inadvertently exposing the test set to a resampling procedure would constitute the
single most severe methodological error a model-improvement experiment of this kind
can make, the notebook does not rely on this claim alone: it computes a
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{D_cm_M2a.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{D_cm_M2b.png}
  \caption{Confusion matrix of M2b (SMOTE on the training set only) on the same
  885-sample held-out test set. The model correctly classifies 201/284
  \textit{Dropout}, 74/159 \textit{Enrolled}, and 334/442 \textit{Graduate}
  students -- an improvement over both M0 and M2a on every class except a
  marginal reduction on \textit{Graduate}.}
  \label{fig:cm-m2b}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Full M2a decision tree (depth 28, 696 leaves). This view is not
  intended to be read node-by-node; it illustrates that reweighting classes via
  \texttt{class\_weight='balanced'} changes which splits the tree prioritises
  but does not meaningfully change the size of the tree, which remains close in
  complexity to M0 (depth 27, 634 leaves).}
...
\begin{figure}[H]
  ...
  \caption{M2b decision tree, truncated to the first four levels. The model
  was trained on the SMOTE-resampled training set (5{,}301 rows) but is
  evaluated exclusively on the original, untouched test set. The true depth of
  the fitted tree is 39, with 847 leaves.}
  \label{fig:tree-m2b}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Full M2b decision tree (depth 39, 847 leaves) -- substantially
  larger than both M0 and M2a, because SMOTE increases the number of training
  rows available to split on (3{,}539 $\rightarrow$ 5{,}301), giving the tree
  more opportunities to fragment the feature space.}
...
of the shared \texttt{results.csv} schema but are read directly from the two
...
Tree depth / leaves   & 27 / 634 & 28 / 696 & \textbf{39 / 847} & -- & -- \\
\bottomrule
\end{tabular}
\end{table}
...
improvement is meant to prioritise either fall or stay flat, there is no evidence
...
omitted, since selectively reporting only favourable configurations would
misrepresent the effectiveness of class weighting for this dataset.

\textbf{M2b is clearly effective.} Recall on \textit{Enrolled} rises from
...
Crucially, an overall test-accuracy \emph{increase} for M2b is a welcome but
secondary finding here; per the assignment's own guidance that overall accuracy
alone is not a sufficient success criterion for a class-imbalance intervention,
the correct basis for judging success is the per-class recall table above, on
which M2b unambiguously outperforms both M0 and M2a for the two classes this
improvement targets.
...
(no category active) rather than any mixture of multiple categories. This fully
explains why \textit{Course}, with the largest number of possible categories,
shows the worst violation rate at $83.0\%$: more categories means a randomly
chosen neighbour is more likely to belong to a different category, which is
precisely the condition that produces a violating (all-zero) synthetic vector
under this truncation mechanism.
...
\citep{imblearn_oversampling_guide}. The group retains vanilla SMOTE for M2b
because it is the technique explicitly required by the assignment brief. The M2b results reported above should
therefore be read together with this limitation: the recall and precision
improvements are genuinely measured on the real, untouched held-out test set, but
...
accuracy for a reduction in bias toward the majority class; when this happens,
...
held-out split: it simultaneously increases test accuracy, \textit{Dropout}
recall, and \textit{Enrolled} recall, at the cost of a $0.90$ percentage-point
reduction in \textit{Graduate} recall. If the deployment priority is detecting
...
additional experiment, and fairness across the dataset's sensitive attributes
should be examined. Within the scope of this experiment and the project's shared
data split, however, M2b answers the improvement question affirmatively: the
minority \textit{Enrolled} class and the \textit{Dropout} class are both
recognised more reliably when SMOTE is applied strictly to the training split.
...
precision from $0.3234$ to $0.4044$ and lowering the overall error rate from
...
of vanilla SMOTE rather than presenting only favourable figures.

% -----------------------------------------------------------------------
% Suggested BibTeX entries for the shared bibliography file, if any of the
% following keys are not already defined elsewhere in the project:
%
...
%   title  = {{SMOTE} -- {API} Reference, imbalanced-learn 0.14.2},
%   year   = {2024},
%   howpublished = {\url{https://imbalanced-learn.org/stable/references/generated/imblearn.over_sampling.SMOTE.html}}
% }
%
% @misc{imblearn_over_sampling_guide,
%   author = {{imbalanced-learn developers}},
%   title  = {Over-sampling -- User Guide, imbalanced-learn 0.14.2},
%   year   = {2024},
%   howpublished = {\url{https://imbalanced-learn.org/stable/over_sampling.html}}
% }
%
...
%   year   = {2024},
%   howpublished = {\url{https://github.com/scikit-learn-contrib/imbalanced-learn/blob/0.14.2/imblearn/utils/_validation.py}}
% }
% -----------------------------------------------------------------------