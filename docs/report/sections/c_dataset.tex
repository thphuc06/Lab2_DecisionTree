...
33 (Biofuel Production Technologies, $n=12$, 66.7\% Dropout) and course
...
sample ($n=12$) and should be read as indicative rather than precise. The
...
either way. It is also available at enrollment time, which is why it
underpins the early-warning model M3 (Section~\ref{sec:f}, f.3).
...
it is only known \emph{after} the second semester ends, so it cannot be
used by the early-warning model M3.
...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    \item \textbf{Keep the raw integer code} for high-cardinality
      ...
      \texttt{Nacionality} (21 values). One-hot encoding these would
      create dozens of sparse columns, which would likely destabilize
      the tree more than it would help -- a limitation the group
  ...
  \item \textbf{Stratified train/test split}: 80\%/20\%, stratified by
    ...
    This matters because the data is imbalanced; a plain random split
    could accidentally starve the test set of the minority class. A
    fixed random seed is used so all 5 models in the project are
    evaluated on the exact same split.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item \textbf{Features split cleanly by collection time} (known at
    ...
    feature-selection experiment with a real application meaning (early
    warning) rather than a purely statistics-driven one.
