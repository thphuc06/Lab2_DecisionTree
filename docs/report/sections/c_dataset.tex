...
% docs/report_draft_b_c.md (section "c. Dataset Description"), AFTER the
% 2026-08-30 fact-check pass that fixed 5 real errors (see progress/A.md).
% Numbers below (course Graduate rates, correlation values, feature
% importance ranks) are the CORRECTED values verified directly against
% data/raw/data.csv and outputs/E_feature_importance_comparison.csv --
% do not "fix" them back to the values in docs/02-DATASET-VA-CONG-VIEC.md
% Phan A5/A6, those were the ones found to be wrong.
...
\begin{itemize}
  \item The raw file contains \textbf{4,424 rows and 37 columns}
    $\rightarrow$ 4{,}424 samples (students), 36 features + 1 label
    column \texttt{Target}.
  ...
  \item \textbf{Notable finding:} the UCI page lists 36 features, but
    ...
    \texttt{Admission grade} --- present in the released CSV but absent
    from the paper's table. This is direct evidence the group inspected
    the real data rather than copying the metadata.
\end{itemize}
...
\textbf{Enrolled} is the minority class and the hardest to classify, as it
sits ``between'' the other two classes in feature space -- this is the
reason the class-imbalance improvement (Section~\ref{sec:f}, f.2) exists.
...
paid on time. Students with a \textbf{scholarship} have a 3$\times$ lower
Dropout rate than those without (12\% vs.\ 39\%).
...
mean a feature is an important tree split once it is placed alongside
stronger, possibly overlapping features such as \texttt{Tuition fees up
to date} -- the tree may be extracting the same signal from a stronger
correlate instead.

By program of study, the two lowest-graduation programs are course code
33 (Biofuel Production Technologies, 66.7\% Dropout) and course code 9119
(Informatics Engineering, 54.1\% Dropout) -- only \textbf{8.3\% and 8.2\%}
of their students graduate respectively, versus the dataset average of
49.9\%. The two
highest-graduation programs are course code 9238 (Social Service, 69.9\%
Graduate) and course code 9500 (Nursing, 71.5\% Graduate), with the
lowest Dropout rates (18.3\% and 15.4\%).
...
Table~\ref{tab:c-importance}, \texttt{Course} ranks 6th -- close to
top-5 either way, and importantly, available at enrollment time, which
is why it underpins the early-warning model M3 (Section~\ref{sec:f},
f.3).
...
(rank~1, importance 0.340 -- more than the sum of ranks 2--5 combined),
but it is only known \emph{after} the second semester ends, so it cannot
be used by the early-warning model M3.
...
\begin{enumerate}
  ...
  \item \textbf{Re-encoding categorical columns stored as integers.} Many
    ...
    = Nursing -- these codes carry no ordinal relationship). Left as-is,
  ...
  \begin{itemize}
    ...
    \item \textbf{Keep the raw integer code} for high-cardinality
      columns: \texttt{Mother's/Father's occupation} (32/46 values),
      \texttt{Mother's/Father's qualification} (29/34),
      \texttt{Nacionality} (21) -- one-hot encoding these would create
      dozens of sparse columns, destabilizing the tree more than it
      would help. This trade-off is noted explicitly as a limitation.
  ...
  \item \textbf{Stratified train/test split}: 80\%/20\%, stratified by
    ...
    required because the data is imbalanced, to avoid a random split
    accidentally starving the test set of the minority class. A fixed
    random seed is used so all 5 models in the project are evaluated on
...
\texttt{Father's occupation}} -- this last pair is \emph{higher} than
the semester-approved pair (\texttt{1st/2nd sem approved}). The pair
...
\begin{quote}
\todo{Data-integrity note (delete once acted on): the multicollinearity
table in \texttt{docs/02-DATASET-VA-CONG-VIEC.md} Section~A6 has two
values that do not match the real data when recomputed: it lists
\texttt{Nacionality}$\leftrightarrow$\texttt{International} as $r=0.912$
(actual $r=0.791$), and \texttt{Mother's/Father's occupation} as
$r=0.724$ (actual $r=0.910$ -- \emph{much higher} than recorded, in fact
the 3rd-highest pair in the dataset, not rank~9 as the old table implies).
This report uses the recomputed values above; \texttt{docs/02} should be
updated so Role E's rationale for dropping columns in f.3 is not built on
the wrong ranking.}
\end{quote}
...
columns and can make the tree structure less stable across runs. This is
a quantitative basis for Role E to consider dropping a redundant column
(e.g.\ \texttt{International}, or one of the two parent-occupation
columns) when extending the feature-selection experiment in
...
\begin{enumerate}
  \item \textbf{Features have clear, real-world semantics} (tuition,
    ...
    split translates into an ordinary sentence -- this plays directly to
    a decision tree's interpretability strength, unlike image or text
    data where that advantage is less exploitable.
  \item \textbf{A mix of categorical and numerical features} -- a tree
    handles both natively without scaling, unlike KNN or linear models
    which require it.
  \item \textbf{A genuine class imbalance} (50\%/32\%/18\%) -- creates a
    realistic scenario to apply and compare imbalance-handling
    techniques, one of the improvement directions the brief itself
    suggests (Section 3.2.d, ``Handling class imbalance'').
  \item \textbf{Features split cleanly by collection time} (known at
    enrollment vs.\ known only at end of semester) -- enables a
    feature-selection experiment with real application meaning (early
    warning), not just a statistics-driven selection.
  \item \textbf{No missing values} -- lets the group spend its time on
    ...
    brief's own emphasis: not just running a ready-made model, but
    clearly explaining how the tree is built and why the improvements
    help.
\end{enumerate}
...
university's sample from 2008--2019}, not causal relationships, and
